/**
 * Nayori commitment profile v1. Kept byte-identical with @perkos/agent-sdk and the Evaluator
 * (`src/evaluation-commitments.ts` in both repositories); the fixture test pins the shared hashes.
...
export type CommitmentNetwork = "testnet" | "mainnet";
...
export interface CriteriaCommitmentInput {
  readonly network: CommitmentNetwork;
...
export const CRITERIA_MARKER = "\nnayori-criteria-v1:";
const EVIDENCE = "ny1:";
const HASH = /^[0-9a-f]{64}$/;

export function isPrincipalForNetwork(value: string, network: CommitmentNetwork): boolean {
  const prefix = network === "testnet" ? "ST" : "SP";
  return value.startsWith(prefix) && !value.includes(".") && validateStacksAddress(value);
}
export function isContractIdForNetwork(value: string, network: CommitmentNetwork): boolean {
...
function criteriaManifest(input: CriteriaCommitmentInput) {
  ...
  text(input.description, 512 - CRITERIA_MARKER.length - 64);
...
export async function prepareEvaluationJob(input: CriteriaCommitmentInput) {
  ...
  return { criteriaHash, description: input.description + CRITERIA_MARKER + criteriaHash };
}
export function parseEvaluationDescription(description: string) {
  text(description, 512);
  const index = description.lastIndexOf(CRITERIA_MARKER);
  ensure(index > 0);
  const plainDescription = description.slice(0, index);
  const criteriaHash = description.slice(index + CRITERIA_MARKER.length);
...
export async function prepareEvaluationSubmission(input: EvidenceCommitmentInput) {
  ...
  ensure(/^[1-9][0-9]{0,38}$/.test(input.jobId) && BigInt(input.jobId) < BigInt("340282366920938463463374607431768211456"));
