;; Nayori STX Agentic Commerce v6 - 2% earned service fee candidate
;; Autonomous evaluator decisions with an immutable per-deployment appeal window.

;; ============================================
;; Constants
;; ============================================
(define-constant ERR_NOT_OWNER (err u800))
(define-constant ERR_NOT_AUTHORIZED (err u801))
(define-constant ERR_JOB_NOT_FOUND (err u802))
(define-constant ERR_INVALID_STATUS (err u803))
(define-constant ERR_JOB_EXPIRED (err u804))
(define-constant ERR_INVALID_BUDGET (err u805))
(define-constant ERR_NOT_CLIENT (err u807))
(define-constant ERR_NOT_PROVIDER (err u808))
(define-constant ERR_NOT_EVALUATOR (err u809))
(define-constant ERR_ALREADY_FUNDED (err u810))
(define-constant ERR_INVALID_DESCRIPTION (err u812))
(define-constant ERR_INVALID_PARTY (err u813))
(define-constant ERR_ALREADY_RATED (err u814))
(define-constant ERR_INVALID_RATING (err u815))
(define-constant ERR_NOT_EXPIRED (err u816))
(define-constant ERR_REVIEW_WINDOW_CLOSED (err u817))
(define-constant ERR_TIMEOUT_TOO_EARLY (err u818))
(define-constant ERR_REVIEW_DEADLINE_MISSING (err u819))
(define-constant ERR_NO_PENDING_OWNER (err u820))
(define-constant ERR_NOT_PENDING_OWNER (err u821))
(define-constant ERR_INVALID_OWNER (err u822))
(define-constant ERR_REPUTATION_NOT_PENDING (err u823))
(define-constant ERR_NOT_CONFIGURED (err u824))
(define-constant ERR_ALREADY_CONFIGURED (err u825))
(define-constant ERR_INVALID_APPEAL_WINDOW (err u826))
(define-constant ERR_INVALID_APPEAL_AUTHORITY (err u827))
(define-constant ERR_INVALID_DECISION (err u828))
(define-constant ERR_DECISION_NOT_FOUND (err u829))
(define-constant ERR_NOT_APPELLANT (err u830))
(define-constant ERR_APPEAL_WINDOW_CLOSED (err u831))
(define-constant ERR_FINALIZE_TOO_EARLY (err u832))
(define-constant ERR_NOT_APPEAL_AUTHORITY (err u833))
(define-constant ERR_INVALID_EVIDENCE_HASH (err u834))
(define-constant ERR_APPEAL_RESOLUTION_CLOSED (err u835))
(define-constant ERR_APPEAL_RESOLUTION_TOO_EARLY (err u836))
(define-constant ERR_RESOLUTION_DEADLINE_MISSING (err u838))
(define-constant ERR_NO_PENDING_APPEAL_AUTHORITY (err u839))
(define-constant ERR_NOT_PENDING_APPEAL_AUTHORITY (err u840))

(define-constant ERR_INVALID_TREASURY (err u841))
(define-constant ERR_SERVICE_FEE_NOT_SETTLED (err u842))
(define-constant ERR_SERVICE_FEE_NOT_WAIVED (err u843))
(define-constant ERR_SERVICE_FEE_NOT_REFUNDABLE (err u844))
(define-constant ERR_NOT_TREASURY (err u845))
(define-constant ERR_SERVICE_FEE_ALREADY_WAIVED (err u846))
(define-constant ERR_ESCROW_MISMATCH (err u847))
(define-constant ERR_TOKEN_TRANSFER_FAILED (err u848))
...
(define-private (settle-service-payment (job-id uint) (recipient principal))
  (let (
    ...
    (if (> fee u0)
      (begin (try! (as-contract (stx-transfer? fee tx-sender treasury))) true)
      true)
...
(define-public (refund-service-fee (job-id uint))
  (let (
    (job (unwrap! (map-get? jobs job-id) ERR_JOB_NOT_FOUND))
...
;; ============================================
;; Two-step ownership and appeal authority
;; ============================================
...
(define-public (finalize-decision (job-id uint))
  (let
    (
      (job (unwrap! (map-get? jobs job-id) ERR_JOB_NOT_FOUND))
      (decision-state (unwrap! (map-get? decisions job-id) ERR_DECISION_NOT_FOUND))
    ...
    (try! (settle-service-payment job-id (if (is-eq decision DECISION_APPROVE) provider client)))
...
(define-public (resolve-appeal
  ...
  (let
    ...
    (try! (settle-service-payment job-id (if (is-eq final-decision DECISION_APPROVE) provider client)))
    (map-delete escrow-balances job-id)
    (map-set jobs job-id (merge job {
...
(define-public (settle-appeal-timeout (job-id uint))
  (let
    (
      (job (unwrap! (map-get? jobs job-id) ERR_JOB_NOT_FOUND))
      (decision-state (unwrap! (map-get? decisions job-id) ERR_DECISION_NOT_FOUND))
...
(define-public (settle-review-timeout (job-id uint))
  (let
    (
      (job (unwrap! (map-get? jobs job-id) ERR_JOB_NOT_FOUND))
      (budget (get budget job))
