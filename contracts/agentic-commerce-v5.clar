;; Nayori STX Agentic Commerce v5
...
;; ============================================
(define-public (initialize-protocol (appeal-window uint) (new-appeal-authority principal))
  (begin
...
(define-public (finalize-decision (job-id uint))
  (let
    ...
    (if (is-eq decision DECISION_APPROVE)
...
(define-public (resolve-appeal
  ...
  (let
    ...
    (if (is-eq final-decision DECISION_APPROVE)
      (try! (as-contract (stx-transfer? budget tx-sender provider)))
      (try! (as-contract (stx-transfer? budget tx-sender client)))
    )
    (map-delete escrow-balances job-id)
